...
class OrganizationRelationshipService
{
    ...
    ): VendorOrganizationRelationship {
        $operationKey ??= 'organization-relationship:'.Str::uuid();

        return DB::transaction(function () use ($relationship, $target, $actor, $reason, $operationKey) {
            $existing = OrganizationRelationshipEvent::where('operation_key', $operationKey)->first();
            if ($existing) {
                return $existing->relationship()->with('organization')->firstOrFail();
            }
            ...
            if (! in_array($target, self::TRANSITIONS[$from] ?? [], true)) {
                throw ValidationException::withMessages([
                    'status' => "Không thể chuyển quan hệ từ {$from} sang {$target}.",
                ]);
            }
            ...
            OrganizationRelationshipEvent::create([
                'vendor_organization_relationship_id' => $locked->id,
                'actor_id' => $actor->id,
                'from_status' => $from,
                'to_status' => $target,
                'reason' => $reason,
