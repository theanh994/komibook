    ...
    public function transition(
        ...
        return DB::transaction(function () use ($agreement, $target, $actor, $reason, $operationKey) {
            ...
            if ($target === 'demo_accepted' && (! $locked->is_demo || $actor->role !== 'admin')) {
                throw ValidationException::withMessages(['status' => 'Chỉ Admin được duyệt mô phỏng cho thỏa thuận đã đánh dấu demo.']);
            }
            if ($target === 'verified' && $locked->is_demo) {
                throw ValidationException::withMessages(['status' => 'Thỏa thuận demo không được đánh dấu là đã xác minh pháp lý.']);
            }
