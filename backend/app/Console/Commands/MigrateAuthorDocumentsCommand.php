...
class MigrateAuthorDocumentsCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:migrate-author-documents {--force : Thực thi cập nhật và di chuyển file thực sự}';
     ...
     * @var string
     */
    protected $description = 'Di chuyển tài liệu CCCD tác giả từ public sang private storage (Mặc định chạy ở chế độ --dry-run)';
    ...
    public function handle(): int
    {
        ...
        if (! $isForce) {
            ...
            $this->info('[FORCE MODE] Bắt đầu di trú tài liệu CCCD tác giả sang private storage...');
        }

        $authors = Author::whereNotNull('identity_document')->get();
        $movedCount = 0;
        $missingCount = 0;
        ...
        foreach ($authors as $author) {
            ...
            if (Storage::disk('public')->exists($path)) {
                $this->line("Phát hiện file public: {$path} (Tác giả ID: {$author->id})");
                ...
                $this->warn("File không tồn tại ở cả đĩa public lẫn private: {$path} (Tác giả ID: {$author->id})");
                $missingCount++;
            }
        }

        $this->info('TỔNG KẾT DI TRÚ TÀI LIỆU CCCD:');
