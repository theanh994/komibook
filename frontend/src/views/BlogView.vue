...
<template>
  <main class="min-h-screen bg-background py-10 text-on-surface md:py-16">
    <section class="mx-auto max-w-[1280px] px-4 md:px-gutter" aria-labelledby="blog-title">
      ...
      <h1 id="blog-title" class="sr-only">KomiBook Editorial - Bài viết mới</h1>

      <div class="mt-10 grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_300px]">
        <!-- Main Articles Column (Left) -->
        <div class="min-w-0">
          <div v-if="loading" class="space-y-6" role="status" aria-label="Đang tải bài viết">
            <div v-for="i in 4" :key="i" class="ui-skeleton h-64"></div>
          </div>
          <div v-else-if="error" class="ui-empty-state" role="alert">
            ...
            <button class="ui-btn ui-btn-secondary" type="button" @click="fetchArticles">Thử lại</button>
          </div>
          <div v-else-if="!articles.length" class="ui-empty-state">
            ...
            <p class="font-bold">Chưa có bài viết phù hợp.</p>
          </div>
          <template v-else>
            <div class="space-y-8">
            ...
            <nav v-if="lastPage > 1" class="mt-10 flex justify-center gap-3" aria-label="Phân trang bản tin">
              <button class="ui-btn ui-btn-secondary" :disabled="page <= 1" @click="page--; fetchArticles()">Trang trước</button>
              <span class="self-center text-sm">Trang {{ page }} / {{ lastPage }}</span>
              <button class="ui-btn ui-btn-secondary" :disabled="page >= lastPage" @click="page++; fetchArticles()">Trang sau</button>
            </nav>
          </template>

          <div class="mt-10 flex justify-center">
            <RouterLink to="/blog/contribute" class="ui-btn ui-btn-secondary no-underline">
              <span class="material-symbols-outlined text-[20px]" aria-hidden="true">rate_review</span>Gửi review của bạn
            </RouterLink>
          </div>
        </div>

        <!-- Sidebar Column (Right: Search Bar + Latest Articles) -->
        <aside class="space-y-6 lg:sticky lg:top-24">
          <!-- 1. Search Box (Above Latest News) -->
          <form class="rounded-xl border border-outline-variant/40 bg-surface-container-lowest p-5 space-y-3.5 shadow-xs" role="search" @submit.prevent="applyFilters">
            <h3 class="text-base font-bold text-primary flex items-center gap-2">
              <span class="material-symbols-outlined text-[#00b14f] text-xl">search</span>
              <span>Tìm kiếm bài viết</span>
            </h3>
            
            <div class="relative min-w-0">
              <label class="sr-only" for="news-search">Tìm trong bản tin</label>
              <span class="material-symbols-outlined pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-outline text-lg" aria-hidden="true">search</span>
              <input id="news-search" v-model.trim="filters.search" class="ui-field pl-10 text-sm w-full" type="search" placeholder="Tìm bài viết…" />
            </div>

            <div>
              <label class="sr-only" for="news-type">Loại bài</label>
              <select id="news-type" v-model="filters.type" class="ui-field text-sm w-full" @change="applyFilters">
                <option v-for="[value,label] in types" :key="value" :value="value">{{ label }}</option>
              </select>
            </div>

            <button class="ui-btn ui-btn-primary w-full flex justify-center items-center gap-2 cursor-pointer" type="submit">
              <span class="material-symbols-outlined text-[18px]" aria-hidden="true">search</span>
              <span>Tìm kiếm</span>
            </button>
          </form>

          <!-- 2. Latest News Box (Below Search Box) -->
          <div class="rounded-xl border border-outline-variant/40 bg-surface-container-lowest p-5 shadow-xs" aria-labelledby="latest-news-title">
            ...
            <div class="divide-y divide-outline-variant/40">
              <RouterLink v-for="item in latestArticles" :key="item.id" :to="`/blog/${item.slug}`" class="grid grid-cols-[68px_minmax(0,1fr)] gap-3 py-4 text-primary no-underline hover:underline">
                <div class="grid h-16 place-items-center overflow-hidden rounded-lg bg-surface-container-low">
                  <img v-if="item.cover_image && !imageFailures.has(item.id)" :src="getCoverUrl(item.cover_image)" :alt="item.title" class="h-full w-full object-contain" loading="lazy" @error="markImageFailed(item.id)" />
                  <span v-else class="material-symbols-outlined text-outline" aria-hidden="true">article</span>
                </div>
                <div class="min-w-0">
                  <h3 class="line-clamp-3 text-sm font-bold leading-5">{{ item.title }}</h3>
                  <p class="mt-1 text-xs font-normal text-on-surface-variant">{{ formatDate(item.published_at) }}</p>
                </div>
              </RouterLink>
            </div>
          </div>
