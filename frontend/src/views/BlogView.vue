<template>
  <div class="min-h-screen bg-background text-on-surface antialiased flex flex-col font-inter">
      ...
      <div class="max-w-[1400px] mx-auto relative z-10">
        <div class="flex flex-col items-center text-center space-y-8">
           <div class="inline-flex items-center gap-3 px-6 py-2.5 rounded-full bg-surface-container-high border border-outline-variant/10 shadow-sm animate-fade-in">
              ...
              <span class="text-[10px] font-bold uppercase tracking-[0.3em] text-primary">KomiBook Editorial</span>
           </div>
           
           <h1 class="text-6xl md:text-8xl font-bold text-on-surface tracking-tighter leading-none animate-fade-in delay-100">
      ...
      <div class="flex flex-col lg:flex-row gap-xl">
        ...
        <div class="flex-1 min-w-0 space-y-xl">
          ...
          <div class="flex gap-3 overflow-x-auto no-scrollbar pb-4 sticky top-24 z-20 bg-background/80 backdrop-blur-md py-2">
            <button 
              ...
              class="px-8 py-4 rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] transition-all border-none cursor-pointer whitespace-nowrap"
            ...
            <div @click="goToPost(featuredPost)" class="md:col-span-8 group relative aspect-[16/9] md:aspect-auto md:h-[600px] rounded-[48px] overflow-hidden border border-outline-variant/10 shadow-2xl cursor-pointer">
               ...
               <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent flex flex-col justify-end p-8 md:p-14">
                  <div class="flex items-center gap-4 mb-6">
                    <span class="px-5 py-2 bg-primary text-on-primary text-[9px] font-bold uppercase tracking-widest rounded-xl">Tiêu điểm</span>
                    <span class="text-white/60 text-[10px] font-bold uppercase tracking-widest">{{ featuredPost.date }}</span>
                  </div>
                  <h2 class="text-3xl md:text-5xl font-bold text-white leading-tight mb-6 group-hover:text-primary transition-colors">{{ featuredPost.title }}</h2>
                  <p class="text-white/70 text-lg max-w-2xl line-clamp-2 font-medium mb-8">{{ featuredPost.excerpt }}</p>
                  <div class="flex items-center gap-6">
                     <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white font-bold">K</div>
                        <span class="text-sm font-bold text-white">Komi Editorial</span>
                     </div>
            ...
            <div class="md:col-span-4 flex flex-col gap-8">
               <div v-for="post in posts.slice(0, 2)" :key="post.id" @click="goToPost(post)" class="flex-1 group relative rounded-[40px] overflow-hidden border border-outline-variant/10 shadow-xl cursor-pointer bg-surface-container-low">
                  ...
                  <div class="absolute inset-0 bg-gradient-to-t from-surface-container-lowest via-transparent to-transparent p-8 flex flex-col justify-end">
                     <span class="text-primary text-[9px] font-bold uppercase tracking-widest mb-2">{{ post.category }}</span>
                     <h3 class="text-xl font-bold text-on-surface leading-tight group-hover:text-primary transition-colors line-clamp-2">{{ post.title }}</h3>
                  </div>
               </div>
            </div>
          </section>
            ...
            >
              ...
              <div class="p-8 flex flex-col flex-1">
                <div class="flex items-center justify-between mb-4">
                  <span class="px-3 py-1 bg-surface-container-high rounded-lg text-[9px] font-bold text-primary uppercase tracking-widest">{{ post.category }}</span>
                  <time class="text-[10px] font-bold text-outline uppercase tracking-widest">{{ post.date }}</time>
                </div>
                <h3 class="text-2xl font-bold text-on-surface mb-4 group-hover:text-primary transition-colors line-clamp-2 leading-tight tracking-tight">{{ post.title }}</h3>
                ...
                <div class="mt-auto pt-6 border-t border-outline-variant/5 flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-2xl bg-surface-container-high border border-outline-variant/10 overflow-hidden shadow-sm">
                      <div class="w-full h-full bg-primary-container flex items-center justify-center text-xs font-bold text-on-primary-container">
          ...
          <div v-if="filteredPosts.length === 0" class="py-32 text-center">
             ...
             <h3 class="text-2xl font-bold text-on-surface mb-2">Không tìm thấy bài viết</h3>
          ...
          <div v-if="filteredPosts.length > 0" class="flex justify-center pt-20">
             <button class="bg-surface-container-highest text-on-surface px-12 py-5 rounded-[24px] font-bold text-xs uppercase tracking-[0.3em] hover:bg-primary hover:text-on-primary shadow-xl transition-all border-none cursor-pointer flex items-center gap-4 group">
               Tải thêm bài viết
        ...
        <aside class="w-full lg:w-80 flex flex-col gap-10">
          ...
          <div class="bg-surface-container-low/40 p-8 rounded-[48px] border border-outline-variant/10">
            <h3 class="text-xl font-bold text-on-surface mb-8 flex items-center gap-3">
            ...
            <div class="space-y-8">
              <div v-for="(post, index) in popularPosts" :key="post.id" class="group cursor-pointer">
                <div class="flex gap-4">
                  <span class="text-3xl font-bold text-outline/10 group-hover:text-primary transition-all">{{ (index + 1).toString().padStart(2, '0') }}</span>
                  <div class="space-y-2">
                    <h4 class="text-sm font-bold text-on-surface leading-tight group-hover:text-primary transition-all line-clamp-2 tracking-tight">{{ post.title }}</h4>
                    <div class="flex items-center gap-3">
                      <time class="text-[9px] font-bold text-outline uppercase tracking-widest">{{ post.date }}</time>
                      <div class="flex items-center gap-1 text-primary">
                         <span class="material-symbols-outlined text-[12px] fill-1">bolt</span>
                         <span class="text-[9px] font-bold uppercase tracking-widest">Hot</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="bg-primary text-on-primary p-10 rounded-[48px] shadow-2xl relative overflow-hidden text-center group">
            ...
            <div class="relative z-10">
              ...
              <h3 class="text-2xl font-bold mb-3 leading-tight tracking-tight">Cộng đồng Komi Editorial</h3>
              ...
              <form class="space-y-4">
                ...
                />
                <button class="w-full bg-white text-primary px-8 py-4 rounded-2xl font-bold text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-2xl border-none cursor-pointer">Gia nhập ngay</button>
              </form>
            </div>
          </div>
          ...
          <!-- Cloud Tags -->
          <div class="bg-surface-container-low/40 p-8 rounded-[48px] border border-outline-variant/10">
            <h3 class="text-xl font-bold text-on-surface mb-6">Tags phổ biến</h3>
            <div class="flex flex-wrap gap-2">
              <span v-for="tag in tags" :key="tag" class="px-5 py-2.5 bg-white border border-outline-variant/10 rounded-xl text-[10px] font-bold text-on-surface-variant hover:bg-primary hover:text-on-primary hover:shadow-lg transition-all cursor-pointer uppercase tracking-widest">
                {{ tag }}
              </span>
            </div>
          </div>
