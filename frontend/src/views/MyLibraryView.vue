    ...
    <div class="w-full px-gutter max-w-[1280px] mx-auto py-xl relative z-10">
      <div class="flex flex-col lg:flex-row gap-xl">
        ...
        <main class="flex-1 min-w-0">
          <div class="bg-surface-container-lowest rounded-3xl border border-outline-variant/30 soft-shadow overflow-hidden min-h-[700px] flex flex-col">
            
            <!-- Hero Header Section (Improved MD3) -->
            <div class="p-lg md:p-xl border-b border-outline-variant/10 bg-surface-container-low/20">
              <div class="flex flex-col gap-10">
                <!-- Row 1: Title & Description -->
                <div class="space-y-4">
                  <div class="flex items-center gap-4">
                     <div class="w-1.5 h-10 bg-primary rounded-full shadow-[0_0_15px_rgba(var(--primary),0.3)]"></div>
                     <h1 class="text-3xl md:text-4xl font-bold text-on-surface tracking-tight leading-tight">Tủ sách cá nhân</h1>
                  </div>
                ...
                <div class="flex flex-wrap items-center justify-between gap-6">
                  ...
                  <div class="flex flex-wrap gap-3">
                     <div class="flex items-center gap-2.5 bg-surface-container-high/60 px-5 py-2.5 rounded-2xl border border-outline-variant/10 shadow-sm">
                        ...
                        <span class="text-[11px] font-bold uppercase tracking-widest">{{ ebookCount }} E-BOOKS</span>
                     </div>
                     <div class="flex items-center gap-2.5 bg-surface-container-high/60 px-5 py-2.5 rounded-2xl border border-outline-variant/10 shadow-sm">
                        <span class="material-symbols-outlined text-[20px] text-secondary fill-1">menu_book</span>
                        <span class="text-[11px] font-bold uppercase tracking-widest">{{ physicalCount }} SÁCH GIẤY</span>
                     </div>
                  </div>
                  ...
                  <div class="flex p-2 bg-surface-container-highest/50 backdrop-blur-md rounded-[28px] border border-outline-variant/10 shadow-inner w-fit">
                    <button 
                      ...
                      class="px-8 py-4 rounded-2xl text-[10px] font-bold transition-all border-none cursor-pointer uppercase tracking-[0.2em] whitespace-nowrap"
            ...
            <div class="p-lg md:p-xl flex-grow">
              <!-- Loading MD3 -->
              <div v-if="loading" class="py-32 flex flex-col items-center gap-8 animate-fade-in">
                ...
                <p class="text-[11px] font-bold text-outline uppercase tracking-[0.3em] animate-pulse">Đang đồng bộ thư viện...</p>
              </div>
              ...
              <div v-else-if="filteredItems.length === 0" class="py-32 text-center animate-fade-in">
                ...
                <h3 class="text-3xl font-bold text-on-surface mb-4 tracking-tight">Kệ sách còn đang trống</h3>
                ...
                <button @click="$router.push('/catalog')" class="bg-primary text-on-primary px-12 py-5 rounded-2xl font-bold text-xs uppercase tracking-[0.2em] shadow-2xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all border-none cursor-pointer flex items-center gap-4 mx-auto">
                  Khám phá KomiBook
              ...
              <div v-else class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-12">
                <div v-for="(item, index) in filteredItems" :key="item.book.id" class="group animate-slide-up" :style="{ animationDelay: (index * 50) + 'ms' }">
                  <div class="relative aspect-[3/4.5] rounded-2xl overflow-hidden shadow-lg group-hover:shadow-[0_40px_80px_rgba(0,0,0,0.15)] group-hover:-translate-y-4 transition-all duration-700 ease-out border border-outline-variant/10 mb-6 bg-surface-container-low">
                    ...
                    <div class="absolute top-4 right-4 z-30">
                       <button 
                         @click.stop="toggleWishlist(item.book.id)" 
                         class="w-10 h-10 rounded-2xl bg-black/40 backdrop-blur-xl border border-white/10 flex items-center justify-center text-white hover:scale-110 active:scale-95 transition-all group/heart shadow-xl"
                         :title="wishlistStore.isFavorite(item.book.id) ? 'Xóa khỏi yêu thích' : 'Thêm vào yêu thích'"
                       >
                          <span 
                            class="material-symbols-outlined text-[22px] transition-all duration-300"
                    ...
                    <!-- Type Indicator Float (Refined) -->
                    <div class="absolute top-4 left-4 z-10">
                       <div class="w-8 h-8 bg-black/40 backdrop-blur-md rounded-full border border-white/10 flex items-center justify-center text-white/90 shadow-lg" :title="item.book.type === 'ebook' ? 'Digital Edition' : 'Hardcover Edition'">
                          <span class="material-symbols-outlined text-[18px]">{{ item.book.type === 'ebook' ? 'bolt' : 'menu_book' }}</span>
                       </div>
                    </div>
                    ...
                    <div v-if="item.book.type === 'ebook'" class="absolute bottom-0 left-0 right-0 h-1 bg-black/20 z-10 overflow-hidden">
                       <div class="h-full bg-primary shadow-[0_0_10px_rgba(var(--primary),0.5)] transition-all duration-1000" :style="{ width: readingProgress + '%' }"></div>
                    </div>
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-primary/90 via-primary/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center p-8 gap-4 z-20">
                      <button 
                        ...
                        class="w-full bg-white text-primary py-4 rounded-2xl font-bold text-xs uppercase tracking-widest shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3 border-none cursor-pointer"
                      >
                      ...
                      <button 
                        ...
                        class="w-full bg-white text-on-surface py-4 rounded-2xl font-bold text-xs uppercase tracking-widest shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3 border-none cursor-pointer"
                      >
                      ...
                      <button 
                        ...
                        class="w-full bg-surface-container-lowest/10 backdrop-blur-xl text-white border border-white/30 py-4 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-white/20 transition-all flex items-center justify-center gap-3 cursor-pointer"
                      >
                        Thông tin
                      </button>
                    </div>
                  ...
                  <div class="px-2 space-y-2 text-center sm:text-left">
                    <h4 class="text-lg font-bold text-on-surface leading-tight line-clamp-1 group-hover:text-primary transition-colors tracking-tight">{{ item.book.title }}</h4>
                    <div class="flex items-center justify-center sm:justify-start gap-2">
                       <p class="text-[10px] text-outline font-bold uppercase tracking-[0.2em] opacity-60">{{ item.book.author || 'KomiBook Author' }}</p>
                    </div>
                    ...
                    <div v-if="item.book.type === 'physical'" class="flex items-center justify-center sm:justify-start gap-2 pt-1">
                       ...
                       <span class="text-[9px] font-bold text-outline uppercase tracking-widest">
            ...
            <div class="p-8 md:p-10 bg-surface-container-low/30 border-t border-outline-variant/5">
              <div class="flex items-center gap-4 bg-primary/5 p-6 rounded-3xl border border-primary/10">
                ...
                <div>
                   <p class="text-sm font-bold text-on-surface mb-0.5">Mẹo nhỏ cho bạn</p>
