...
<template>
  <main id="main-content" class="space-y-6" tabindex="-1">
    <header class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p class="text-sm font-semibold uppercase tracking-wider text-primary">Vận hành kho</p><h1 class="mt-1 text-3xl font-bold text-on-surface">Tồn kho & vị trí kệ</h1></div>
    ...
    <section v-else class="overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest">
      <div class="hidden grid-cols-[minmax(240px,1fr)_140px_180px] gap-4 border-b border-outline-variant bg-surface-container px-5 py-3 text-sm font-semibold md:grid"><span>Sách</span><span>Số lượng</span><span>Vị trí kệ</span></div>
      <article v-for="stock in stocks" :key="stock.id" class="grid gap-3 border-b border-outline-variant/50 p-5 last:border-0 md:grid-cols-[minmax(240px,1fr)_140px_180px] md:items-center">
        ...
        <div><span class="text-xs text-on-surface-variant md:hidden">Vị trí kệ</span><p>{{ stock.shelf_location || 'Chưa gán' }}</p></div>
      </article>
    </section>
  </main>
</template>
